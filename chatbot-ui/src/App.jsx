...
export default function App() {
  ...
  const processAudio = async (audioBlob) => {
    ...
    try {
      ...
      addReply(text, routed_category ? { model, category: routed_category } : null, rag_chunks || null);
    } catch (e) {
  ...
  const submitTextPrompt = async () => {
    ...
    try {
      ...
      addReply(text, routed_category ? { model, category: routed_category } : null, data.rag_chunks || null);
    } catch (e) {
  ...
  const addReply = (text, modelInfo = null, ragChunks = null) => {
    setConversation((p) => [...p, { role: "model", parts: [{ text }], modelInfo, ragChunks }]);
            ...
            <div className={`group relative max-w-lg p-4 rounded-2xl ${msg.role === "user" ? "bg-blue-600" : "bg-gray-700"}`}>
              {msg.parts[0].type === "audio" ? (
                ...
                <>
                  ...
                  {/* RAG sources & snippets */}
                  {msg.ragChunks && msg.ragChunks.length > 0 && (
                    <details className="mt-3 border-t border-gray-600 pt-2">
                      <summary className="text-xs text-indigo-300 cursor-pointer hover:text-indigo-200 select-none">
                        📚 Fuentes ({msg.ragChunks.length})
                      </summary>
                      <div className="mt-2 space-y-2">
                        {msg.ragChunks.map((chunk, idx) => (
                          <div key={idx} className="p-2 bg-gray-800/80 rounded text-xs">
                            <div className="flex items-center justify-between text-gray-400 mb-1">
                              <span className="font-semibold text-gray-300">{chunk.source || 'Documento'}</span>
                              {chunk.relevance !== undefined && (
                                <span className="text-indigo-400">{(chunk.relevance * 100).toFixed(0)}%</span>
                              )}
                            </div>
                            {chunk.section && (
                              <div className="text-gray-500 mb-1 italic">{chunk.section}</div>
                            )}
